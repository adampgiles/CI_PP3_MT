...
def register():
    """
    This function registers an account for a user
    in the users database collection. Once
    registered the user is taken to their My Tales
    page and a login session is created. The user
    is redirected back to the register page,
    if the username already exists or both password
...
def login():
    """
    This function checks the user account exists
    and logs in the user if the username and
    password match. The user is redirected to
    the login page, if the username and/or
    password are incorrect. Once logged in the
    ...
    if request.method == "POST":
        ...
        if existing_user:
            if check_password_hash(
                ...
                return redirect(url_for("tales.mytales",
