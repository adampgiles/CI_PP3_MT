...
def get_tales():
    """
    This function displays all tales in the tales
    collection on the base template. The tales can
    be filtered using a search bar to display tales
    ...
    if session.get("logged_in") is True:
        liked = mongo.db.users.find_one(
            {"username": session["user"]})["liked_tales"]
        session["liked"] = liked
...
def clear_search():
    """
    This function clears the search bar and loads
    the tales page with all tales.
    """
    session["search"] = ""
    tales = mongo.db.tales.find()
...
def mytales(username):
    """
    This function displays all of the currently
    logged in user's tales in the tales collection
    on the base template.
    """
    username = mongo.db.users.find_one({
        "username": session["user"]})["username"]
    tales = mongo.db.tales.find({"tale_author": session["user"]})
    return render_template("tales/mytales.html",
                           username=username, tales=tales)


@tales.route("/newtale", methods=["GET", "POST"])
def newtale():
    """
    This function displays a form that the user
    can use to submit a new tale to the tales
    collection. The user is redirected to their
...
def edittale(_id):
    """
    This function displays a pre-populated form
    that the user can use to edit one of their
    existing tales.Changes will then update the
    existing tales collection document. The user
    is redirected to their My Tales page, once
    ...
    if request.method == "POST":
        ...
        mongo.db.users.update_many(
            {"liked_tales": _id}, {"$pull": {"liked_tales": _id}})
...
def deletetale(_id):
    """
    This function takes the user to a page where they
    can confirm if they are sure about deleting their
    existing tale.
    """
    _id = _id
    tale = mongo.db.tales.find_one({"_id": ObjectId(_id)})
...
def confirmdeletetale(_id):
    ...
    removing the document from the collection. The
    user is redirected to their My Tales page,
    once this change is actioned.
    """
    _id = _id
...
def tale(_id):
    """
    This function displays the full tale that was
    selected on the tales page.
    """
    _id = _id
    mongo.db.tales.update_one({
        "_id": ObjectId(_id)}, {"$inc": {"tale_views": 1}})
    tale = mongo.db.tales.find_one({"_id": ObjectId(_id)})
    if session.get("logged_in") is True:
        liked = mongo.db.users.find_one({
            "username": session["user"]})["liked_tales"]
...
def like_tale(_id):
    ...
    increase/decrease a tale's like count by a
    factor of 1. The tale's id is stored in the
    current user's collection document, to
    ensure the user can only like a tale once.
    """
    _id = _id
    liked = mongo.db.users.find_one({
        "username": session["user"]})["liked_tales"]
    if _id in liked:
        mongo.db.users.update_one({
            "username": session["user"]}, {"$pull": {"liked_tales": _id}})
        mongo.db.tales.update_one({
            "_id": ObjectId(_id)}, {"$inc": {"tale_likes": -1}})
        if session.get("fulltale") is True:
            mongo.db.tales.update_one({
                "_id": ObjectId(_id)}, {"$inc": {"tale_views": -1}})
    else:
        mongo.db.users.update_one({
            "username": session["user"]}, {"$push": {"liked_tales": _id}})
        mongo.db.tales.update_one({
            "_id": ObjectId(_id)}, {"$inc": {"tale_likes": 1}})
        if session.get("fulltale") is True:
            mongo.db.tales.update_one({
